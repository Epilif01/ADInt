...
class Book(Base):
    ...
    def __repr__(self):
        return "<Book(place_id='%s', user_id='%s', timeofcheckin='%s', timeofcheckout='%s')>" % (
...
def new_checkin(place_id, user_id, timeofcheckin, timeofcheckout):
    book = Book(place_id=place_id, user_id=user_id, timeofcheckin=timeofcheckin, timeofcheckout=timeofcheckout)
    session.add(book)
    session.commit()

def checkout(place_id, user_id, timeofcheckout):
    book = session.query(Book).filter_by(place_id=place_id, user_id=user_id, timeofcheckout=None).first()
    if book is None:
